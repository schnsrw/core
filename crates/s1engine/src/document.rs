...
impl Document {
    ...
    ///
    /// Regenerates `content.xml` from the model via the existing ODT
    /// writer, then replaces just that part inside a clone of the
    /// preserved ODF package. Every other part — `styles.xml`,
    /// `meta.xml`, `settings.xml`, `META-INF/manifest.xml`,
    /// `Pictures/*`, `Configurations2/*`, `Thumbnails/*` — rides
    /// through unchanged.
    ///
    /// Limitation (Phase 2a): unknown ODF inside `content.xml` itself
    /// is **not** preserved across edits. Phase 2b will add the
    /// NodeId-keyed body merge for ODT (mirror of `BodyOrigin` for
    /// DOCX).
    #[cfg(feature = "odt")]
    fn export_odt_spliced(&self, pkg: &s1_odf::Package) -> Result<Vec<u8>, Error> {
        ...
        // Step 2: parse the regenerated bytes to extract its content.xml.
        let regenerated_pkg = s1_odf::Package::parse(&regenerated_bytes)
            .map_err(|e| Error::Format(format!("re-parse regenerated odt: {e}")))?;
        let new_content_part = regenerated_pkg
            .parts
            .get("content.xml")
            .ok_or_else(|| Error::Format("regenerated ODT has no content.xml".to_owned()))?
            .clone();

        // Step 3: clone the preserved package and swap content.xml in.
        let mut patched = pkg.clone();
        patched
            .parts
            .insert("content.xml".to_owned(), new_content_part);
