...
//!
//! Designed to round-trip ODF XML parts (`content.xml`, `styles.xml`,
//! `meta.xml`, `settings.xml`, `META-INF/manifest.xml`) through
//! `parse → write → parse` with structural equality. The goal is not
//! byte-perfect output (some normalisation is unavoidable when
//! round-tripping through any XML library) but no information loss:
//! every element, attribute, namespace declaration, text node, CDATA
//! section, comment, and processing instruction is captured in document
//! order.
//!
//! This is a direct mirror of `s1-ooxml::xml` — the AST and serialiser
//! are format-agnostic; the divergence between OOXML and ODF lives in
//! `package.rs`.
    ...
    pub fn parse(input: &str) -> Result<Self> {
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(_) => {}
                Err(e) => {
                    return Err(crate::error::OdfError::Xml {
        ...
        let root = root.ok_or_else(|| {
            crate::error::OdfError::Malformed("XML document has no root element".to_owned())
        })?;
...
fn parse_element(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Eof) => {
                return Err(crate::error::OdfError::Malformed(format!(
